...
| ResNet18 | CIFAR-100 | Sign-FGSM（唯一有效） | ↓9.53%（相对-10.5%） | -4.06% | 2.35 |
...
5. **CIFAR-100 新发现（防御边界条件）**：
   ...
   - ResNet18 × 5000样本：基线 MIA 高达 90.73%，Sign-FGSM 是唯一有效方法（MIA↓9.53%）
   - 复杂损失（KL+AdvReg）在高难度场景反而失效，不如 Sign-FGSM 直接有效
   - **规律总结**：任务越难、过拟合越严重 → 复杂组合失效、简单强力方法反而占优
